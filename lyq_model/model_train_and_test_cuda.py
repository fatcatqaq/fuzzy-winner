...
def train(model, train_loader, dayid, device='cuda', save_dir='./', resume=''):
    # 初始化混合精度训练
    ...
    for i, batch in enumerate(train_loader):
        sample = {k: v.to(device, non_blocking=True) for k, v in batch.items()}
...
def test(model, test_loader, dayid, sample=1.0, device='cuda', resume=''):
    if resume != '':
        checkpoint = torch.load(resume, map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
